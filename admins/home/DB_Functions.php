...
function AlterTable($dbc,$tableName,$FOREIGNKEYS)
{
	$query = "ALTER TABLE '$tableName'";
	foreach ($FOREIGNKEYS as $key => $foreignKey)
	{
		$query .= "ADD CONSTRAINT `$key` FOREIGN KEY (`{$foreignKey['column']}`) REFERENCES `{$foreignKey['refTable']}` (`{$foreignKey['refColumn']}`), ";
	}
	 // Remove the trailing comma and add COMMIT;
	 $query = rtrim($query, ', ') . ";\nCOMMIT;";

	 @mysqli_query($dbc, $query)
	 or die("DB Error: Could not alter the table $tableName! <br>" . @mysqli_error($dbc));
 
	 print "<p>Table $tableName altered successfully.</p>";
}
?>